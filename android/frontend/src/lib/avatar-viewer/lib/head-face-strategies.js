...
export const FACE_MODES = ['natural', 'faithful'];
...
function extractFaceFeatures(frameData) {
  ...
  return {
    ...
    mouthCornerDownRightRatio: (mouthRightCorner.y - noseBase.y) / faceScale,
    mouthCornerDownLeftRatio: (mouthLeftCorner.y - noseBase.y) / faceScale,
...
export function buildSequenceFaceCalibrationFromFrames(frameDataList) {
  ...
  return {
    ...
    mouth: {
      ...
      cornerDownLeft: {
        neutral: quantile(values('mouthCornerDownLeftRatio'), 0.25),
        low: quantile(values('mouthCornerDownLeftRatio'), 0.1),
        high: quantile(values('mouthCornerDownLeftRatio'), 0.9),
      },
      cornerDownRight: {
        neutral: quantile(values('mouthCornerDownRightRatio'), 0.25),
        low: quantile(values('mouthCornerDownRightRatio'), 0.1),
        high: quantile(values('mouthCornerDownRightRatio'), 0.9),
      },
      upperLeft: {
...
function applyFaceCalibration(features, calibration) {
  ...
  const centerSmileLeftRaw = mouth?.cornerLeft
  ...
  const centerSmileRightRaw = mouth?.cornerRight
  ...
  const absoluteSmileLeftGate = remapClamped(features.mouthCornerLiftLeftRatio, 0.006, 0.035);
  const absoluteSmileRightGate = remapClamped(features.mouthCornerLiftRightRatio, 0.006, 0.035);
  const centerFrownLeftRaw = mouth?.cornerLeft
  ...
  const centerFrownRightRaw = mouth?.cornerRight
  ...
  const cornerDownLeftRaw = mouth?.cornerDownLeft
    ? positiveRelative(features.mouthCornerDownLeftRatio, mouth.cornerDownLeft.neutral, mouth.cornerDownLeft.high, 0.012)
    : remapClamped(features.mouthCornerDownLeftRatio - 0.17, 0, 0.045);
  const cornerDownRightRaw = mouth?.cornerDownRight
    ? positiveRelative(features.mouthCornerDownRightRatio, mouth.cornerDownRight.neutral, mouth.cornerDownRight.high, 0.012)
    : remapClamped(features.mouthCornerDownRightRatio - 0.17, 0, 0.045);
  const cornerDownLeftEvidence = cornerDownLeftRaw * (1 - mouthWidthWideRaw * 0.65);
  const cornerDownRightEvidence = cornerDownRightRaw * (1 - mouthWidthWideRaw * 0.65);
  const smileDownSuppressLeft = remapClamped(cornerDownLeftEvidence, 0.16, 0.46);
  const smileDownSuppressRight = remapClamped(cornerDownRightEvidence, 0.16, 0.46);
  const oppositeCornerDropSuppressLeft = Math.max(
    centerFrownRightRaw,
    remapClamped(-features.mouthCornerLiftRightRatio, 0.018, 0.06),
  );
  const oppositeCornerDropSuppressRight = Math.max(
    centerFrownLeftRaw,
    remapClamped(-features.mouthCornerLiftLeftRatio, 0.018, 0.06),
  );
  const smileLeftRaw = centerSmileLeftRaw
    * absoluteSmileLeftGate
    * (1 - smileDownSuppressLeft * 0.96)
    * (1 - oppositeCornerDropSuppressLeft * 0.95);
  const smileRightRaw = centerSmileRightRaw
    * absoluteSmileRightGate
    * (1 - smileDownSuppressRight * 0.96)
    * (1 - oppositeCornerDropSuppressRight * 0.95);
  const frownLeftRaw = Math.max(centerFrownLeftRaw, cornerDownLeftEvidence * 0.78);
  const frownRightRaw = Math.max(centerFrownRightRaw, cornerDownRightEvidence * 0.78);
  ...
  return {
    ...
    cornerDownLeftRaw: clamp01(cornerDownLeftRaw),
    cornerDownRightRaw: clamp01(cornerDownRightRaw),
    cornerDownLeftEvidence: clamp01(cornerDownLeftEvidence),
    cornerDownRightEvidence: clamp01(cornerDownRightEvidence),
    centerSmileLeftRaw: clamp01(centerSmileLeftRaw),
    centerSmileRightRaw: clamp01(centerSmileRightRaw),
    absoluteSmileLeftGate: clamp01(absoluteSmileLeftGate),
    absoluteSmileRightGate: clamp01(absoluteSmileRightGate),
    oppositeCornerDropSuppressLeft: clamp01(oppositeCornerDropSuppressLeft),
    oppositeCornerDropSuppressRight: clamp01(oppositeCornerDropSuppressRight),
...
function solveFaceMorphs(faceSignals, mode, options = {}) {
  ...
  const roundedMouthEvidence = clamp01(
    faceSignals.roundRaw * (0.25 + faceSignals.mouthWidthNarrowRaw * 0.75) +
    faceSignals.protrusionRaw * 0.35,
  );
  const roundedMouthGate = remapClamped(roundedMouthEvidence, 0.28, 0.62);
  const mouthOpenRaw = clamp01(
    (faceSignals.slightOpenRaw * 0.78 + faceSignals.lipSeparationRaw * 0.22) *
    (1 - roundedMouthGate * 0.85),
  );
  const jawOpenRaw = clamp01(
    (faceSignals.jawDropRaw * 0.72 + faceSignals.lipSeparationRaw * 0.08) *
    (1 - roundedMouthGate * 0.9),
  );
  ...
  const mouthPuckerRaw = clamp01(
    (
      roundedMouthEvidence * 0.72 +
      faceSignals.mouthWidthNarrowRaw * 0.2 +
      faceSignals.protrusionRaw * 0.08
    ) * (1 - jawOpenRaw * 0.15),
  );
  const mouthFunnelRaw = clamp01(
    (
      roundedMouthEvidence * 0.48 +
      faceSignals.roundRaw * 0.24 +
      faceSignals.lipSeparationRaw * 0.12
    ) * (1 - jawOpenRaw * 0.1),
  );
...
export function computeHeadFaceStrategy(frameData, options = {}) {
  const mode = FACE_MODES.includes(options.mode) ? options.mode : DEFAULT_FACE_MODE;
  const headFeatures = extractHeadFeatures(frameData);
  const faceFeatures = extractFaceFeatures(frameData);
  ...
  const morphs = calibratedFace ? solveFaceMorphs(calibratedFace, mode, {
    blinkSync: options.blinkSync,
  }) : zeroMorphMap();
  ...
  return {
    ...
    debug: {
      ...
      faceSignals: calibratedFace ? {
        ...
        smileLeftRaw: calibratedFace.smileLeftRaw,
        smileRightRaw: calibratedFace.smileRightRaw,
        frownLeftRaw: calibratedFace.frownLeftRaw,
        frownRightRaw: calibratedFace.frownRightRaw,
        cornerDownLeftRaw: calibratedFace.cornerDownLeftRaw,
        cornerDownRightRaw: calibratedFace.cornerDownRightRaw,
        cornerDownLeftEvidence: calibratedFace.cornerDownLeftEvidence,
        cornerDownRightEvidence: calibratedFace.cornerDownRightEvidence,
        centerSmileLeftRaw: calibratedFace.centerSmileLeftRaw,
        centerSmileRightRaw: calibratedFace.centerSmileRightRaw,
        absoluteSmileLeftGate: calibratedFace.absoluteSmileLeftGate,
        absoluteSmileRightGate: calibratedFace.absoluteSmileRightGate,
        oppositeCornerDropSuppressLeft: calibratedFace.oppositeCornerDropSuppressLeft,
        oppositeCornerDropSuppressRight: calibratedFace.oppositeCornerDropSuppressRight,
      } : null,
    },
  };
}
