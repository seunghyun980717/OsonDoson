...
import type { AudioPayload } from '@/types/ws';
...
// audio.url(상대 경로) → 절대 URL. WS base에서 ws→http로 변환해 같은 호스트의 정적 파일을 가리킨다.
const resolveAudioSrc = (relativeUrl: string): string | null => {
  const wsBase = import.meta.env.VITE_WS_BASE;
  if (!wsBase) {
    console.error('[VoicePlayer] VITE_WS_BASE 환경변수가 설정되어 있지 않음');
    return null;
  }
  const httpBase = wsBase.replace(/^ws/, 'http');
  return new URL(relativeUrl, httpBase).toString();
};

export const VoicePlayer = ({ audio, playbackSpeed = 1, autoPlay = false, replayNonce }: VoicePlayerProps) => {
  ...
  useEffect(() => {
    ...
    const src = resolveAudioSrc(audio.url);
    if (!src) return;
    audioEl.src = src;
  ...
  // 재생/시간/duration 이벤트 리스너 등록 (브라우저 이벤트 콜백이라 set-state-in-effect 룰 통과)
  useEffect(() => {
    const audioEl = audioRef.current;
    if (!audioEl) return;
    ...
    <div className="border-border-light bg-surface-screen flex items-center gap-5 rounded-2xl border px-6 py-6 shadow-[0_8px_24px_rgba(0,0,0,0.06),0_2px_6px_rgba(0,0,0,0.04)]">
      ...
      >
        {playing ? (
          ...
          <Play size={28} fill="white" stroke="white" strokeWidth={1.5} className="translate-x-[1px]" />
        )}
      </button>
