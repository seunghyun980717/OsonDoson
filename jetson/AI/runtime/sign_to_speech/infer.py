...
입력: numpy keypoint (T, 134)  ← 영상→keypoint 변환은 별도 모듈 담당
출력: 글로스 시퀀스 → 한국어 텍스트 → 음성 파일

실행:
    # 프레임별 keypoint JSON 파일들이 있는 폴더
    python -m sign_to_speech.infer --json_dir path/to/frame_jsons/

    # 단일 JSON (프레임 배열 또는 단일 프레임)
    python -m sign_to_speech.infer --json path/to/keypoints.json

    # TTS 없이
    python -m sign_to_speech.infer --json_dir path/to/frame_jsons/ --no_tts
"""
import argparse
import tempfile
...
from core.data_utils.keypoint_loader import parse_frame, normalize_sequence
...
def infer(
    ...
    """
    Args:
        keypoints: (T, 134) float32  ← 외부에서 추출해서 넘겨줌
...
def keypoints_from_json_dir(json_dir: Path) -> np.ndarray:
    """
    프레임별 keypoint JSON 파일 폴더 → (T, 134).
    ...
    frames = [parse_frame(json.loads(f.read_text(encoding="utf-8"))) for f in files]
...
def keypoints_from_json_file(json_path: Path) -> np.ndarray:
    ...
    단일 JSON 파일 → (T, 134).
    파일 구조 자동 감지:
      - 프레임 배열: [{"people": ...}, {"people": ...}, ...]
      - 단일 프레임: {"people": ...}
    """
    import json
    data = json.loads(json_path.read_text(encoding="utf-8"))
    if isinstance(data, list):
        frames = [parse_frame(d) for d in data]
    else:
