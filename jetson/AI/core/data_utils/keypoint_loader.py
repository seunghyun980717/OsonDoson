...
from core.config import IMG_W, IMG_H, CACHE_DIR, KEYPOINT_ZIP_PREFIX

# zip 기반 AIHUB 데이터는 OpenPose 25pt 포맷 (134-dim) 고정.
# KEYPOINT_DIM(현재 150)과 독립적으로 유지.
_OPENPOSE_DIM = 134
_OPENPOSE_PTS = 67
...
def normalize_sequence(seq: np.ndarray) -> np.ndarray:
    ...
    AIHUB zip 포맷 전용 — KEYPOINT_DIM(150)과 무관하게 134-dim 고정.
    """
    T = len(seq)
    out = seq.copy()
    ...
    out_2d = out.reshape(T, _OPENPOSE_PTS, 2)
    out_2d[:, :, 0] = (out_2d[:, :, 0] - center[:, 0:1]) / width
    out_2d[:, :, 1] = (out_2d[:, :, 1] - center[:, 1:2]) / width
    return out_2d.reshape(T, _OPENPOSE_DIM)
