...
from core.config import IMG_W, IMG_H
...
def mediapipe_to_frame(mp_data: dict, mirror_x: bool = True) -> np.ndarray:
    ...
    Args:
        ...
        mirror_x: True면 X 좌우 반전 적용 (Java converter 경유 시),
                  False면 미적용 (AIHub 학습 데이터와 동일 좌표계 유지).
...
# ── sign-v2: MediaPipe → (150,) 직접 변환 ────────────────────────────────────

def mediapipe_to_frame_v2(mp_data: dict) -> np.ndarray:
    """
    MediaPipe Holistic 1프레임 → (150,) float32 (sign-v2 포맷).

    OpenPose 25pt 변환 없이 MediaPipe 33pt pose를 그대로 사용.
    포맷: pose 33pt×2 (66) + lhand 21pt×2 (42) + rhand 21pt×2 (42) = 150

    Args:
        mp_data: {"poseLandmarks": [...33개], "leftHandLandmarks": [...21개], "rightHandLandmarks": [...21개]}

    Returns:
        (150,) float32 — shoulder normalization 적용 전
    """
...
def frames_to_sequence_v2(frames: list[dict]) -> np.ndarray:
    """
    signer_keypoints 프레임 리스트 → (T, 150) shoulder-normalized float32 (sign-v2).

    Args:
        frames: [{"poseLandmarks": [...], "leftHandLandmarks": [...], ...}, ...]

    Returns:
        (T, 150) float32
    """
    from core.config import MP_LSHOULDER, MP_RSHOULDER, LOWER_BODY_START
    ...
    # 하체 포인트 제로 (pose point 23~32 = dim 46~65)
