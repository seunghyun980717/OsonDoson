#!/usr/bin/env python3
"""IK-style postprocess for v0.5 hand-lifting viewer QA files.
...
original estimated_3d unchanged. The v2 correction is chain-relative: low
confidence frames are blended toward the previous frame, finger joints are
corrected using the previous bone as the local reference, PIP/DIP sidebend and
flexion-plane drift are softly limited, and bone lengths are normalized last.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--include-pattern", default="mlp_v0_5_QA_full_*.json")
    parser.add_argument("--output-prefix", default="post_v0_5_ik_QA_full")
...
def process_file(task: tuple[Path, argparse.Namespace]) -> dict[str, Any]:
    ...
    spaces["postprocessed_3d"] = {
        "available": True,
        "coordinate_space": "viewer_normalized_xy_with_hand_lifting_mlp_v0_5_ik_postprocess",
    }
    processing = sample.setdefault("processing", {})
    processing["postprocessed_3d_method"] = "hand_lifting_v0_5_ik_postprocess_v2"
    ...
    if args.write_copy:
        ...
        payload["viewer_qa_alias"] = {
            ...
            "purpose": "hand_lifting_v0_5_ik_postprocessed_3d_viewer_qa",
