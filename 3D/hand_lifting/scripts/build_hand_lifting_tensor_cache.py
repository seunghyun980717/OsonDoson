#!/usr/bin/env python3
"""Build tensor cache shards for hand lifting MLP training.
...
from train_hand_lifting_mlp_v0 import (
    GROUP_INDICES,
    HAND_ORDER,
    INPUT_LAYOUT,
    TARGET_LAYOUT,
...
def summarize_tensors(mask: torch.Tensor, kind: torch.Tensor) -> dict[str, Any]:
    ...
    mask_view = mask.view(row_count, 2, 21)
    valid_counts: dict[str, int] = {}
    valid_ratios: dict[str, float | None] = {}
    for hand_index, hand in enumerate(HAND_ORDER):
        for group, indices in GROUP_INDICES.items():
            count = int(mask_view[:, hand_index, list(indices)].sum().item())
            denominator = row_count * len(indices)
            key = f"{hand}.{group}"
