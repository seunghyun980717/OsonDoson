#!/usr/bin/env python3
"""Train the v0 frame-wise MLP for 2D-to-depth hand lifting.

This is a pipeline validation baseline, not the final temporal model. It reads
the sequence-level train/val/test split JSONL shards, trains a masked L1 MLP,
and writes checkpoint/profile/benchmark artifacts under 3D/hand_lifting/runs.
"""
...
GROUP_INDICES = {
    "wrist": (0,),
    "palm": (5, 9, 13, 17),
    "finger": (1, 2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15, 16, 18, 19, 20),
}
INPUT_LAYOUT = {
...
DEFAULT_SPLIT_ROOT = Path("D:/ssafy/3_\uc790\uc728/artifacts/hand_lifting/05_split/mixed")
DEFAULT_TENSOR_CACHE_ROOT = Path("D:/ssafy/3_\uc790\uc728/artifacts/hand_lifting/06_tensor_cache/mixed")
DEFAULT_REPO_HAND_LIFTING_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_DIR = DEFAULT_REPO_HAND_LIFTING_ROOT / "runs" / "v0_mlp"
...
def row_to_sample(row: dict[str, Any]) -> dict[str, torch.Tensor]:
    ...
    target = torch.zeros((2, 21), dtype=torch.float32)
    mask = torch.zeros((2, 21), dtype=torch.bool)
    labels = row["labels"]
    masks = row["masks"]

    for hand_index, hand in enumerate(HAND_ORDER):
        wrist_root = labels["hand_wrist_root_z"].get(hand)
        relative_z = labels["hand_wrist_relative_z"].get(hand) or []
        hand_mask = masks.get(hand) or {}
        for joint_index in range(21):
            rel = relative_z[joint_index] if joint_index < len(relative_z) else None
            if joint_index == 0:
                target_value = wrist_root
            elif is_finite(wrist_root) and is_finite(rel):
                target_value = float(wrist_root) + float(rel)
            else:
                target_value = None
            if is_finite(target_value):
                target[hand_index, joint_index] = float(target_value)
            else:
                continue
            if joint_index in GROUP_INDICES["wrist"]:
                use_target = bool(hand_mask.get("use_wrist_depth"))
            elif joint_index in GROUP_INDICES["palm"]:
                use_target = bool(hand_mask.get("use_palm_depth"))
            else:
                use_target = bool(hand_mask.get("use_finger_depth"))
            mask[hand_index, joint_index] = use_target
...
def group_mask(mask: torch.Tensor, group: str) -> torch.Tensor:
    view = mask.view(mask.shape[0], 2, 21)
    indices = torch.tensor(GROUP_INDICES[group], device=mask.device)
    return view.index_select(dim=2, index=indices).reshape(mask.shape[0], -1)


def group_values(values: torch.Tensor, group: str) -> torch.Tensor:
    view = values.view(values.shape[0], 2, 21)
    indices = torch.tensor(GROUP_INDICES[group], device=values.device)
    return view.index_select(dim=2, index=indices).reshape(values.shape[0], -1)


def masked_l1_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, dict[str, float]]:
    losses: list[torch.Tensor] = []
    details: dict[str, float] = {}
    abs_error = (pred - target).abs()
    finite = torch.isfinite(target)
    active_mask = mask & finite
    for group in ("wrist", "palm", "finger"):
...
class MetricAccumulator:
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, kind: torch.Tensor) -> None:
        with torch.no_grad():
            ...
            view_error = abs_error.view(abs_error.shape[0], 2, 21)
            view_mask = active_mask.view(active_mask.shape[0], 2, 21)
            for hand_index, hand in enumerate(hand_names):
                for group, indices in GROUP_INDICES.items():
                    selected_error = view_error[:, hand_index, list(indices)]
                    selected_mask = view_mask[:, hand_index, list(indices)]
    ...
    def result(self) -> dict[str, Any]:
        ...
        group_maes: dict[str, list[float]] = {"wrist": [], "palm": [], "finger": []}
...
def main() -> int:
    ...
    train_config_path = args.output_dir / "hand_lifting_v0_train_config.json"
    ...
    history = []
    best_path = args.output_dir / "hand_lifting_v0_mlp_best.pt"
    last_path = args.output_dir / "hand_lifting_v0_mlp_last.pt"
    ...
    write_json(args.output_dir / "hand_lifting_v0_benchmark_val.json", val_benchmark)
    ...
    if not args.no_test:
        ...
        write_json(args.output_dir / "hand_lifting_v0_benchmark_test.json", test_benchmark)
    ...
    write_json(args.output_dir / "hand_lifting_v0_profile.json", profile)
