#!/usr/bin/env python3
"""Create word-viewer QA JSON files with v0.5 MLP estimated_3d blocks.
...
This script creates copied QA words named ``mlp_v0_5_QA_*`` and injects
arm+hand v0.5 MLP z estimates into ``sample.keypoints.estimated_3d``.
...
from train_hand_lifting_mlp_v0_5 import (  # noqa: E402
    HAND_ORDER,
    INPUT_LAYOUT,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=THREE_D_ROOT / "hand_lifting" / "runs" / "v0_5_mlp" / "hand_lifting_v0_5_mlp_best.pt",
        help="v0.5 MLP best checkpoint path.",
    )
    parser.add_argument(
        "--words",
        nargs="+",
    ...
    parser.add_argument("--prefix", default="mlp_v0_5_QA", help="Generated QA filename/title prefix.")
...
def build_estimated_word(
    ...
    for frame_index in range(frame_count):
        ...
        if x_values is None:
            ...
            with torch.no_grad():
                ...
                pred = model(x_tensor).detach().cpu().reshape(-1).tolist()
            ...
            for label, pose_index in (
                ...
                z = shoulder_center_z + float(pred[TARGET_INDEX_BY_NAME[label]]) * shoulder_width_3d
            ...
            for hand in HAND_ORDER:
                ...
                for joint_index in range(1, HAND_JOINT_COUNT):
                    ...
                    z = shoulder_center_z + float(pred[TARGET_INDEX_BY_NAME[key]]) * shoulder_width_3d
    ...
    spaces["estimated_3d"] = {
        "available": True,
        "coordinate_space": "viewer_normalized_xy_with_hand_lifting_mlp_v0_5_z",
    }
    spaces["depth_source"] = "estimated_3d"
    processing["estimated_3d_method"] = "hand_lifting_mlp_v0_5"
    ...
    payload["viewer_qa_alias"] = {
        ...
        "purpose": "hand_lifting_v0_5_estimated_3d_word_viewer_qa",
