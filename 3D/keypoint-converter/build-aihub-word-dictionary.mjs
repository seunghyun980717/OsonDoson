...
const REAL_WORD_Z_POLARITY = 1;
const DEFAULT_KEYPOINT_ROOT = 'D:/ssafy/3_자율/수어 영상/1.Training';
const DEFAULT_MORPHEME_ROOT = 'D:/ssafy/3_자율/수어 영상/1.Training/[라벨]01_real_word_morpheme/morpheme';
const DEFAULT_OUTPUT_DIR = path.resolve(__dirname, '../data/words-aihub-staging');
...
function printUsage() {
  console.log(`Usage:
  node build-aihub-word-dictionary.mjs [options]

Options:
  --keypoint-root <dir>   AIHub training root or a word keypoint root. Defaults to local 1.Training path.
  --morpheme-root <dir>   AIHub word morpheme root. Defaults to local training word morpheme path.
  --output <dir>          Output dictionary directory. Defaults to ../data/words-aihub-staging.
...
async function readJson(filePath) {
  ...
  for (const encoding of ['utf-8', 'euc-kr']) {
    try {
      ...
      candidates.push({ encoding, text, json: JSON.parse(text), score: hangulScore(text) });
    } catch {
...
async function findSequenceRefs(keypointRoot, morphemeRoot) {
  const keypointRoots = await findWordKeypointRoots(keypointRoot);
  const refs = [];
  ...
  for (const root of keypointRoots) {
    ...
    for (const splitDir of splitDirs) {
      ...
      const sequenceDirs = (await fs.readdir(splitPath, { withFileTypes: true }))
        .filter((entry) => entry.isDirectory() && entry.name.startsWith('NIA_SL_WORD'))
        .sort((a, b) => a.name.localeCompare(b.name, 'ko'));
...
async function findWordKeypointRoots(keypointRoot) {
  const rootName = path.basename(keypointRoot);

  if (rootName.endsWith('_real_word_keypoint')) {
    return [keypointRoot];
  }

  const entries = await fs.readdir(keypointRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isDirectory() && entry.name.endsWith('_real_word_keypoint'))
...
function buildClip({ ref, morpheme, fps, segment, segmentIndex, wordIndex, gloss, range, frames }) {
  return {
    ...
    processing: {
      ...
      z_polarity: REAL_WORD_Z_POLARITY,
...
function buildViewerPeople(people) {
  ...
  const parts3d = normalizeViewerPeople3D(people, REAL_WORD_Z_POLARITY);
...
function mergeSamples(results, options, report) {
  ...
  for (const result of results) {
    ...
    for (const sample of result.samples) {
      ...
      samplesByGloss.set(sample.gloss, sample);
    }
  }

  return samplesByGloss;
}
...
async function writeSamples(samplesByGloss, options, report) {
  await ensureDir(options.outputDir);
  const written = [];

  for (const [gloss, sample] of [...samplesByGloss.entries()].sort(([a], [b]) => a.localeCompare(b, 'ko'))) {
    const filePath = path.join(options.outputDir, `${sanitizeFileName(gloss)}.json`);
  ...
  report.written_glosses = written.length;
  return written;
}
...
async function writeReport(report, outputDir) {
  ...
  const reportPath = path.join(reportDir, `aihub-word-dictionary-report-${timestampForFile()}.json`);
