...
function solveFaceMorphs(faceSignals, mode) {
  const config = STRATEGY_CONFIG[mode].morph;
  const blinkLeft = styleSignal(faceSignals.blinkLeftRaw, {
    deadzone: config.blinkDeadzone,
    exponent: config.blinkExponent,
    scale: config.blinkScale,
  });
  const blinkRight = styleSignal(faceSignals.blinkRightRaw, {
    deadzone: config.blinkDeadzone,
    exponent: config.blinkExponent,
    scale: config.blinkScale,
  });
  ...
  return {
    ...
    eyeWideLeft,
    eyeWideRight,
    browInnerUp: styleSignal(faceSignals.browInnerUpRaw, {
      deadzone: config.browDeadzone,
      exponent: 1.2,
      scale: config.browScale,
    }),
    browOuterUpLeft: styleSignal(faceSignals.browOuterUpLeftRaw, {
      deadzone: config.browDeadzone,
      exponent: 1.2,
      scale: config.browScale,
    }),
    browOuterUpRight: styleSignal(faceSignals.browOuterUpRightRaw, {
      deadzone: config.browDeadzone,
      exponent: 1.2,
      scale: config.browScale,
    }),
    browDownLeft: styleSignal(faceSignals.browDownLeftRaw, {
      deadzone: config.browDeadzone,
      exponent: 1.15,
      scale: config.browScale * 0.8,
    }),
    browDownRight: styleSignal(faceSignals.browDownRightRaw, {
      deadzone: config.browDeadzone,
      exponent: 1.15,
      scale: config.browScale * 0.8,
    }),
...
export function computeHeadFaceStrategy(frameData, options = {}) {
  ...
  const morphs = calibratedFace ? solveFaceMorphs(calibratedFace, mode) : zeroMorphMap();
